...
SELECT brand_name, m.name, founded FROM models AS m LEFT JOIN brands as b ON m.brand_name=b.name
WHERE year = '1960';
...
-- Left joins join all contents of a table regardless of whether the rows are matching.
-- Inner joins return rows that exist in both tables.
...
SELECT brand_name FROM (SELECT brand_name, COUNT(name) AS count FROM models GROUP BY 1) AS query WHERE count > 5;

-- 2. Add the following rows to the Models table.


.. FIXME
    example here has name and brand_name backwards. Instructions are ambiguous. -- you're tellin me!
...
.. FIXME
  ...
  IIHS Safety Award    2015      the id for the 2015 Subaru Outback
    ...
    FROM 
        models 
    WHERE
        name = 'Outback' AND year = 2015;

-- 5. Using a subquery, select only the *name* of any model whose
-- year is the same year that *any* brand was founded.
SELECT name FROM models 
WHERE EXISTS (SELECT founded FROM brands WHERE models.year = brands.founded);
